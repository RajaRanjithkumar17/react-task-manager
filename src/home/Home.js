...
const Home = () => {
  ...
  const fillterByascen = () => {
    console.log("search", searc_Task, state);

    if (searc_Task?.length!==0) {
      console.log(
        state?.tasks.filter((elm) => {
          elm.tasktitle.includes(searc_Task);
        }),
        typeof searc_Task
      );

      const result = state?.tasks.filter((elm) => {
        elm.tasktitle.includes(searc_Task);
      });
  ...
  return (
    <div>
      ...
      {state?.tasks?.length === 0 && <h5 style={{textAlign:"center" }} > Add New Task...</h5>}
